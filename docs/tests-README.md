...
* **Lyrics provider tests (`test_lyrics_providers.py`)**: the fetch chain in `scripts/lyrics/lyrics.py` against stubbed HTTP - the lrc and ttml parsers, the search validation that killed the Darkside-for-Faded mismatch (artist must match, a known duration must agree within five seconds), BetterLyrics' community API answering first with plain-format rows refused (unsynced words are not an answer for a widget whose whole point is the sync), and lrclib's exact get beating its search.
