...
| `modules/imi/bar/UpdatesCount.qml` | ~~spawns the upgrade terminal, then a 5s post-upgrade outcome state machine + `notify-send`~~ — **done**: `services/Updates.qml` owns `runUpgrade()`, `checkNow()`, the outcome timer and `outcomeFor(count)`; the widget holds no Process or Timer (`test_updates_contract.py` pins it) |
