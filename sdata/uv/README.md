...
For summary:
- They will be installed to the virtual environment `$IMMATERIAL_IMPULSE_VIRTUAL_ENV`.
- The default value of `$IMMATERIAL_IMPULSE_VIRTUAL_ENV` is `$XDG_STATE_HOME/quickshell/.venv`.
  - The default value of `$XDG_STATE_HOME` is `$HOME/.local/state`.
- Currently we use `env = IMMATERIAL_IMPULSE_VIRTUAL_ENV, ~/.local/state/quickshell/.venv` in `~/.config/hypr/hyprland/env.conf` to set this environment variable.[^2]
...
```bash
source $(eval echo $IMMATERIAL_IMPULSE_VIRTUAL_ENV)/bin/activate
```
...
Example: In `~/‎.config/quickshell/imi/screenshot.qml`:
```qml
Process {
...
```python
#!/usr/bin/env -S\_/bin/sh\_-c\_"source\_\$(eval\_echo\_\$IMMATERIAL_IMPULSE_VIRTUAL_ENV)/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
```
And that's it!
...
In this example, in `~/‎.config/quickshell/imi/screenshot.qml` we should modify `find_regions.py` to the wrapper script `find-regions-venv.sh`:
```qml
Process {
id: imageDetectionProcess
