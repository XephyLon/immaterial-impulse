...
here. The engine is driven end to end in `test_modes_runtime.py`. 130cee4ec ("feat(modes): the Modes & Routines engine, ported from the p3drovfx fork"). The editor's controls are this shell's, never a second implementation: text fields on `ToolbarTextField` (`PlainField` / `TimeField` / `NumberField` keep only commit semantics), removable chips on `FilterChip`, every switch row on `ConfigSwitch` (`EditorSwitchRow` is it on the card tier; `EditorRow` draws with `CatalogueRow`), a form's segmented choice a LABELLED `FormChoice` at the form's width, and one `EditorPopup` plate under the kind menu, the icon picker and the suggestion menu. What remains under `modules/imi/modes/` (`SmallButton`, `FooterButton`, `FormIconButton`, `FormLabel`, `FormHint`, `AddRowButton`) is a role wrapper over `RippleButton` / `StyledText` with the editor's sizes, registered as such in `lint_component_gallery.py`. A hand-rolled Rectangle-plus-StyledTextInput field or a `StyledSwitch` beside a label is the fork kit coming back. 974621970 ("refactor(modes): the editor's fields, chips and popups are the shell's"), 94017f93d ("refactor(modes): every switch row in the editor is the shell's ConfigSwitch").
