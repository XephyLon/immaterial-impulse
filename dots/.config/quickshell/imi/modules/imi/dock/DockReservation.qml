 ...
 * reconfigured. What still steps is the zone: floating reserves the lift too,
 * from the CONFIGURED state - written once at the start of a direction, and
 * the compositor re-tiles on its own window animation. The compositor adds
 * the margin to the zone by itself. It lives here, on the dock's side, rather
...
Singleton {
    readonly property real zone: DockGeometry.exclusiveZone(
        ...
        + DockGeometry.splitZoneExtra(FrameGeometry.enabled, FrameGeometry.dockAttached,
