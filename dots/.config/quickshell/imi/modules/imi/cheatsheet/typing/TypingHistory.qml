...
Singleton {
    ...
    function registerStart() {
        if (!Config.options.cheatsheet.typingTest.history.enable)
            return;
    ...
    function registerFinish(result) {
        if (!Config.options.cheatsheet.typingTest.history.enable || result.duration < 1)
            return;
    ...
    function record(result) {
        ...
        const limit = Math.max(1, Math.min(500, Config.options.cheatsheet.typingTest.history.maxEntries));
