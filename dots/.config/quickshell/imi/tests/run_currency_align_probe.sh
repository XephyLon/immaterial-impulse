...
# the movement block's registration against its value, measured and shot.
set -u
SOCKET="wl-imi-currency-align"
TMP=$(mktemp -d)
...
WPID=$!
sleep 2
DBUS_SESSION_BUS_ADDRESS="unix:path=/nonexistent" WAYLAND_DISPLAY="$SOCKET" CURRENCY_ALIGN_SHOTS="${CURRENCY_ALIGN_SHOTS:-}" timeout 60 qs -p "$(dirname "$0")/../CurrencyAlignProbe.qml" 2>&1 | grep "CurrencyAlign"
