...
set -u
SOCKET="wayland-imi-media-tree-motion"
TMP=$(mktemp -d)
...
WPID=$!
sleep 2
WAYLAND_DISPLAY="$SOCKET" MEDIA_PROBE_SHOTS="${MEDIA_PROBE_SHOTS:-}" timeout 40 qs -p "$(dirname "$0")/../MediaTreeMotionProbe.qml" 2>&1 | grep "MediaTreeMotion"
