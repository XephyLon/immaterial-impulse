...
/**
 * The phone (imi's PhoneConnect) is reachable (`reachable: true`, default) or out
...
ModeCondition {
    ...
    readonly property bool paired: PhoneConnect.available && !!PhoneConnect.activeDevice
    readonly property bool reachable: root.paired && PhoneConnect.activeDevice.reachable === true
    readonly property int charge: Number(PhoneConnect.activeDevice?.battery?.charge ?? PhoneConnect.activeDevice?.charge ?? -1)
    ...
    reason: {
        ...
        const name = PhoneConnect.activeDevice?.name ?? "phone";
