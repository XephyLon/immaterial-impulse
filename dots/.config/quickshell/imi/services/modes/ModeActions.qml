...
QtObject {
    ...
    // imi switches dark/light through the wallpaper script (the same call
    // AutoTheme and the AI's set_color_scheme make); there is no service.
    ...
    readonly property var registry: ({
        ...
        darkMode: {
            ...
            apply: v => {
                ...
                root.setDarkMode(dark);
            },
            revert: was => { root.setDarkMode(!(was === "light" || was === false)); }
        },
        brightness: {
        ...
        vpn: {
            ...
            editor: "switch", volatile: false,
            // imi's Vpn service: NetworkManager's vpn/wireguard profiles. "On"
            // activates the first profile (or the one already active).
            available: () => Vpn.enableService && (Vpn.connections?.length ?? 0) > 0,
            read: () => Vpn.anyActive,
            normalize: v => !!v,
            apply: v => {
                if (!!v === Vpn.anyActive)
                    return;
                if (v)
                    Vpn.setConnectionActive(Vpn.connections[0].name, true);
                else
                    Vpn.deactivateAll();
            },
            revert: was => {
                if (!!was === Vpn.anyActive)
                    return;
                if (was)
                    Vpn.setConnectionActive(Vpn.connections[0].name, true);
                else
        ...
        tailscale: {
            ...
            editor: "switch", volatile: false,
            available: () => Tailscale.enableService && Tailscale.available,
            read: () => Tailscale.running,
            normalize: v => !!v,
            apply: v => { if (!!v !== Tailscale.running) Tailscale.toggle(); },
            revert: was => { if (!!was !== Tailscale.running) Tailscale.toggle(); }
        },

        pingPhone: {
            id: "pingPhone", category: "radios", label: "Phone", icon: "smartphone",
            editor: "phone", volatile: false,
            // value: { kind: "ping" | "ring", message }, through imi's PhoneConnect
            available: () => PhoneConnect.available && PhoneConnect.canPing,
            apply: v => {
                ...
                if (req.kind === "ring")
                    PhoneConnect.ring(device);
                else
                    PhoneConnect.ping(device);
            }
        },
